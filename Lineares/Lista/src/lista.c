...
static int lista_redimensionar(Lista *lista, size_t nova_capacidade) {
    ...
    int *novo_buffer = memoria_realocar_vetor(lista->dados, nova_capacidade, sizeof(*lista->dados));
    if (novo_buffer == NULL) {
        return 0;
    }
...
int lista_criar(Lista *lista, size_t capacidade_inicial) {
    ...
    lista->dados = memoria_alocar_vetor_zero(capacidade_inicial, sizeof(*lista->dados));
    if (lista->dados == NULL) {
        return 0;
    }

...
UnionFind *union_find_criar(size_t tamanho) {
    ...
    uf->pai = memoria_alocar_vetor(tamanho, sizeof(*uf->pai));
    uf->rank = memoria_alocar_vetor_zero(tamanho, sizeof(*uf->rank));
...
int kruskal(size_t vertices,
    ...
    *peso_total = 0;
    ...
    ArestaPeso *ordenadas = memoria_alocar_vetor(quantidade_arestas, sizeof(*ordenadas));
    if (ordenadas == NULL) {
        return 0;
    }
...
int prim_matriz(size_t vertices,
    ...
    *peso_total = 0;
    ...
    int *chave = memoria_alocar_vetor(vertices, sizeof(*chave));
    size_t *pai = memoria_alocar_vetor(vertices, sizeof(*pai));
    int *incluido = memoria_alocar_vetor_zero(vertices, sizeof(*incluido));

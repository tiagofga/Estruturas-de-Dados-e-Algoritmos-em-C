...
static int heap_redimensionar(Heap *heap, size_t nova_capacidade) {
    ...
    novo_buffer = memoria_realocar_vetor(heap->dados, nova_capacidade, sizeof(*heap->dados));
    if (novo_buffer == NULL) {
        return 0;
    }
...
int heap_criar(Heap *heap, size_t capacidade_inicial) {
    ...
    heap->dados = memoria_alocar_vetor_zero(capacidade_inicial, sizeof(*heap->dados));
    if (heap->dados == NULL) {
        return 0;
    }
